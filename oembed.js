...
extend(Oembed.prototype, {
	...
	embedElement: function(element) {
		...
		if (embedAction) {
			self.settings.onEmbed = embedAction;
		} else if (!self.settings.onEmbed) {
			self.settings.onEmbed = function (oembedData) {
				window.oembed.insertCode(element, self.settings.embedMethod, oembedData);
			};
		}
				...
				if (resourceURL.match(regExp) !== null) {
					...
					var ajaxopts = extend({
						...
						success: function (data) {
							...
							if (provider !== null) {
								provider.params = getNormalizedParams(self.settings[provider.name]) || {};
								css(provider, {
